...
CREATE TABLE Chat_History (
    ...
    timestamp DATETIME NOT NULL,           -- Cột A: Thời gian
    user_message NVARCHAR(MAX) NOT NULL,   -- Cột B: Câu hỏi khách hàng
    ai_response NVARCHAR(MAX) NOT NULL,    -- Cột C: Phản hồi AI
    session_id NVARCHAR(255) NOT NULL      -- Cột D: Session ID
);
INSERT INTO Chat_History(
timestamp,user_message,ai_response,session_id)
Values('2025-06-08 15:27:50', N'tạo sự kiên xem phim 19h tối nay',N'Bạn đã thêm lịch hẹn xem phim vào lúc 19h tối nay rồi mà.  Bạn muốn thêm thông tin gì khác vào sự kiện này không? Ví dụ như tên phim, địa điểm xem phim, hoặc thay đổi thời gian?','SCHEDULE_SESSION_1751769774159');
CREATE TABLE Email_Reminder (
    id INT IDENTITY PRIMARY KEY,
    id_event INT FOREIGN KEY REFERENCES UserEvents(id_event),
    reminder_time DATETIME,
    [sent] BIT DEFAULT 0
);
drop table Email_Reminder
CREATE TABLE Email_Reminder (
    id INT IDENTITY(1,1) PRIMARY KEY,
    id_event INT NOT NULL,
    reminder_time DATETIME NOT NULL,
    email_sent BIT NOT NULL DEFAULT 0,
    CONSTRAINT fk_event FOREIGN KEY (id_event) REFERENCES UserEvents(id_event)
);
select * from Chat_History
select * from Email_Reminder
